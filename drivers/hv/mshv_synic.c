...
static bool mshv_async_call_completion_isr(struct hv_message *msg)
{
	...
	partition_dbg(partition, "completing async hypercall\n");
	...
	while (true) {
		...
		while (true) {
			...
			if (unlikely(vp_index >= MSHV_MAX_VPS)) {
				partition_err(partition, "%s: VP index %u out of bounds\n",
					      __func__, vp_index);
				goto unlock_out;
			}
			...
			if (unlikely(!vp)) {
				partition_err(partition, "%s: failed to find vp\n",
					      __func__);
				goto unlock_out;
			}
...
mshv_intercept_isr(struct hv_message *msg)
{
	...
	if (msg->header.message_type != HVMSG_OPAQUE_INTERCEPT) {
		partition_err(partition, "%s: wrong message type %d", __func__,
...
int
mshv_register_doorbell(struct mshv_partition *partition, doorbell_cb_t doorbell_cb,
		       u64 gpa, u64 val, u64 flags)
{
	...
	port_table_info->port_doorbell.data = (void *)partition;
	ret = mshv_portid_alloc(port_table_info);
	if (ret < 0) {
		partition_err(partition, "Failed to create the doorbell port!\n");
	...
	ret = hv_call_create_port(hv_current_partition_id, port_id, partition->id,
				  &port_info,
				  0, 0, NUMA_NO_NODE);

	if (ret < 0) {
		partition_err(partition, "Failed to create the port!\n");
	...
	ret = hv_call_connect_port(hv_current_partition_id, port_id, partition->id,
...
int
mshv_unregister_doorbell(struct mshv_partition *partition, int doorbell_portid)
{
	int ret = 0;
	union hv_port_id port_id = { 0 };
	union hv_connection_id connection_id = { 0 };
	...
	ret = hv_call_disconnect_port(partition->id, connection_id);
	if (ret < 0)
		partition_err(partition, "Failed to disconnect the doorbell connection!\n");
